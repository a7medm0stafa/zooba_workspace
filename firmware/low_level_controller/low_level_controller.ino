...
const float INTERNAL_GEAR_RATIO =
    44.727; // Internal gearbox reduction (measured)
const float EXTERNAL_GEAR_RATIO =
    45.45 / 16.35; // External herringbone pair (45.45mm / 16.35mm) = 2.7798
...
float PI_INTEGRAL_MAX =
    400.0;           // Anti-windup limit (increased to allow 100% PWM)
...
// ==================== Servo Constants ====================
const int SERVO_CENTER = 78;
const int SERVO_MIN = 33;
...
void updateYaw(float dt) {
  ...
  // Gyro integration (yaw rate around Z axis, convert back to degrees for
  // filter)
  float gyroYawRate = imuGyroZ / DEG_TO_RAD; // rad/s → °/s

  // Complementary filter: blend gyro integration with accumulated yaw
  // (No magnetometer on HW-123, so no absolute reference — gyro-only
  // integration
