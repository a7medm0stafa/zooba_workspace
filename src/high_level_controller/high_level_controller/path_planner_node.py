...
def get_track_waypoints(track_name, start_x=0.0, start_y=0.0):
    ...
    if track_name == 'track_1':
        ...
        # Vehicle starts at (0, 1.75) heading East (θ=0)
        # Lane center coordinates:
        #   Top straight:    Y = 1.75  (between inner 1.5 and outer 2.0)
        #   Bottom straight: Y = -1.75
        ...
        #   Left/Right: Y from -1.0 to 1.0  (2m box centered at Y=0)
        # Corner arc centers at (±1.5, ±1.0), lane radius = 0.75m

        wps = []

        # Start: middle of top straight, heading East (+X)
        # Go along top straight towards +X
        wps.append((0.0, 1.75))        # START
        wps.append((1.0, 1.75))        # mid top straight
        wps.append((1.5, 1.75))        # end of top straight

        # Top-right corner: center (1.5, 1.0), R=0.75
        # Arc from 90° to 0° (clockwise = decreasing angle)
        wps += _arc_waypoints(1.5, 1.0, 0.75,
                              math.radians(90), math.radians(0), n_pts=8)

        # Right straight: X=2.25, going south from Y=1.0 to Y=-1.0
        wps.append((2.25, 0.5))
        wps.append((2.25, 0.0))
        wps.append((2.25, -0.5))
        wps.append((2.25, -1.0))

        # Bottom-right corner: center (1.5, -1.0), R=0.75
        # Arc from 0° to -90° (clockwise)
        wps += _arc_waypoints(1.5, -1.0, 0.75,
                              math.radians(0), math.radians(-90), n_pts=8)

        # Bottom straight: Y=-1.75, going west from X=1.5 to X=-1.5
        wps.append((1.0, -1.75))
        wps.append((0.0, -1.75))
        wps.append((-1.0, -1.75))
        wps.append((-1.5, -1.75))

        # Bottom-left corner: center (-1.5, -1.0), R=0.75
        # Arc from -90° to -180° (clockwise)
        wps += _arc_waypoints(-1.5, -1.0, 0.75,
                              math.radians(-90), math.radians(-180), n_pts=8)

        # Left straight: X=-2.25, going north from Y=-1.0 to Y=1.0
        wps.append((-2.25, -0.5))
        wps.append((-2.25, 0.0))
        wps.append((-2.25, 0.5))
        wps.append((-2.25, 1.0))

        # Top-left corner: center (-1.5, 1.0), R=0.75
        ...
        wps.append((-1.0, 1.75))
        wps.append((0.0, 1.75))        # BACK TO START (close the loop)
